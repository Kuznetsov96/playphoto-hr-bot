...
describe("тексты, показываемые всплывающей плашкой", () => {
     ...
     */
    /**
     * Оба пути ошибки в ответе на офер обязаны вести себя одинаково: и когда
     * бэкенд не ответил, и когда у сотрудницы нет канонического id, она видит
     * одно и то же. Разные тексты на один и тот же для неё случай — «кнопка не
     * сработала» — читались бы как разные неполадки.
     */
    it("оба пути ошибки офера показывают один текст", () => {
        expect(STAFF_TEXTS["staff-replacement-offer-error-alert"]).toContain("підтримку");
    });
