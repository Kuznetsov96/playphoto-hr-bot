...
export const PING_CONFIG = {
    ...
    /**
     * Потолок напоминаний: через сколько после САМОЙ РАССЫЛКИ пинги прекращаются.
     *
     * До этого цикл был бесконечным — `nextPingAt` двигался вперёд без счётчика,
     * и человек, который не отвечает, получал напоминание каждые 4 часа до конца
     * сбора. У молчания всегда есть причина, которую бот исправить не может:
     * человек увольняется и это ещё не отметили, лежит в больнице, потерял
     * телефон. Во всех трёх случаях правильное действие одно — перестать
     * долбить и показать имя владельцу, чтобы тот позвонил.
     *
     * Отсчёт от `broadcast.createdAt`, а не по числу отправленных пингов: это
     * не требует нового поля в схеме (миграция на проде — самая дорогая правка
     * здесь), и окно читается по-человечески — «трое суток», а не «двенадцать
     * напоминаний». При интервале в 4 часа и старте через 2 суток окно даёт
     * около шести напоминаний: достаточно, чтобы достучаться до забывчивого,
     * и не превращает бота в преследователя для остальных.
     */
    MAX_PING_AGE_MS: 5 * 24 * 60 * 60 * 1000 // 5 days from the broadcast
};
