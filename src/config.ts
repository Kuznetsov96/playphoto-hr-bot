...
export const PING_CONFIG = {
    ...
    /**
     * Тихие часы по Киеву: раньше `QUIET_UNTIL_HOUR` и с `QUIET_FROM_HOUR`
     * напоминания не уходят, а переносятся на ближайшее утро.
     *
     * Интервал в 4 часа ровно укладывается в сутки, поэтому сдвига не было:
     * человек получал пинг в 02:00 КАЖДУЮ ночь, пока не ответит. Половина
     * напоминаний приходилась на нерабочее время — а это ровно то, из-за чего
     * уведомления от бота отключают совсем, после чего напоминания перестают
     * работать и для всех остальных.
     *
     * 10:00 — не новое число: в это же время уходит сама рассылка 23-го
     * (`monthly-preferences-trigger.ts`), то есть кодовая база уже считает его
     * приличным часом для контакта.
     */
    QUIET_FROM_HOUR: 22,
    QUIET_UNTIL_HOUR: 10,
