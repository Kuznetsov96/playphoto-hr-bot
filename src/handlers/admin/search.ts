...
adminSearchHandlers.on("message:text", async (ctx, next) => {
    ...
    if (step.startsWith("admin_reply_direct_")) {
        ...
        try {
            ...
            // 1. Deliver to User
            await ctx.api.sendMessage(Number(targetTgId), `💌 <b>Message from Admin:</b>\n\n${messageText}`, { parse_mode: "HTML" });
...
async function handleAdminMessageSend(ctx: MyContext, userId: string, messageTextStr: string) {
    ...
    try {
        await ctx.api.sendMessage(Number(user.telegramId), escapeHtml(messageTextStr), { parse_mode: "HTML" });
