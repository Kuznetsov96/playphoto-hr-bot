...
adminCandidateMenu.dynamic(async (ctx, range) => {
    ...
    if (isMentor && cand.status === "DISCOVERY_SCHEDULED") {
        range.text("✅ Discovery Passed", async (ctx) => {
            ...
            logBusinessEvent({
                event: "admin.candidate.discovery.marked_passed",
                ...
                stage: "DISCOVERY",
                result: "success",
                module: "admin-recruitment",
                operation: "completeDiscovery",
                safeContext: { fromStatus: cand.status }
            });
            ...
            logBusinessEvent({
                event: "admin.candidate.discovery.marked_failed",
                correlationId: ctx.correlationId,
                updateId: ctx.update.update_id,
                telegramId: ctx.from?.id,
                candidateId: cand.id,
    ...
    if (isMentor && cand.status === "TRAINING_SCHEDULED") {
        range.text("✅ Training Completed", async (ctx) => {
            ...
            logBusinessEvent({
                event: "admin.candidate.training.marked_passed",
                correlationId: ctx.correlationId,
                updateId: ctx.update.update_id,
                telegramId: ctx.from?.id,
                candidateId: cand.id,
            ...
            logBusinessEvent({
                event: "admin.candidate.training.marked_failed",
                correlationId: ctx.correlationId,
                updateId: ctx.update.update_id,
                telegramId: ctx.from?.id,
                candidateId: cand.id,
    ...
    if (isHr && ["SCREENING", "WAITLIST", "WAITLIST_HR"].includes(cand.status)) {
        range.text("🗓️ Re-invite to Interview", async (ctx) => {
            ...
            logAuditEvent({
                event: "admin.candidate.reinvited_to_interview",
                correlationId: ctx.correlationId,
                updateId: ctx.update.update_id,
                telegramId: ctx.from?.id,
                candidateId: candId,
    ...
    if (isSuper && isReadyForStaging && (cand.status === CandidateStatus.TRAINING_COMPLETED || cand.status === CandidateStatus.OFFLINE_STAGING) && !cand.notificationSent) {
        range.text("🚀 Confirm & Notify Staging", async (ctx) => {
            ...
            if (result && 'error' in result) {
                logAuditEvent({
                    event: "admin.candidate.staging_notified",
                    correlationId: ctx.correlationId,
                    updateId: ctx.update.update_id,
                    telegramId: ctx.from?.id,
                    candidateId: cand.id,
