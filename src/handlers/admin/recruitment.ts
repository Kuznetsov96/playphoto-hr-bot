...
async function renderHiringNeedDetails(ctx: MyContext, item: HiringNeedItem, options: { pushToStack?: boolean } = {}) {
    ...
    await ScreenManager.renderScreen(
        ctx,
        hiringNeedsService.formatNeedDetail(item),
        buildHiringNeedDetailKeyboard(item),
        renderOptions
    );
}
...
adminHiringNeedDetailsMenu.dynamic(async (ctx, range) => {
    ...
    range.text(ADMIN_TEXTS["hr-menu-back"], async (ctx) => {
        ...
        await ScreenManager.goBack(ctx, hiringNeedsService.formatBoardText(freshBoard, "ADMIN"), "admin-hiring-needs");
    });
});
...
adminRecruitmentHandlers.callbackQuery("admin_hn_back", async (ctx) => {
    const freshBoard = await hiringNeedsService.getBoard();
    await ScreenManager.goBack(ctx, hiringNeedsService.formatBoardText(freshBoard, "ADMIN"), "admin-hiring-needs");
    await ctx.answerCallbackQuery().catch(() => { });
});
...
adminCandidateMenu.dynamic(async (ctx, range) => {
    ...
    const canEditStaging = isSuper && (cand.status === "OFFLINE_STAGING" || cand.status === "AWAITING_FIRST_SHIFT");
