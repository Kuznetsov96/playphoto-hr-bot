    ...
    try {
        ...
        if (payload?.startsWith("bcq_")) {
            ...
            if (!isNaN(broadcastId)) {
                ...
                await ctx.reply(isStaff
                    ? "<b>Є запитання щодо останнього повідомлення?</b>\n\nНапиши детальніше просто сюди або натисни кнопку нижче — розберемося."
                    : "<b>Маєте запитання щодо останнього повідомлення?</b>\n\nНапишіть детальніше просто сюди або натисніть кнопку нижче — ми розберемося.", {
                    parse_mode: "HTML",
                    reply_markup: kb
                });
                return;
