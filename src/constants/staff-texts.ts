export const STAFF_TEXTS = {
  /**
   * Доступ до закритої бази знань — каналу з матеріалами по роботі.
   *
  ...
  /**
   * У меню співробітниці стоїть у ряду з іншими пунктами, де в кожного є свій
   * значок, — без емодзі кнопка читалася б як недороблена. Це той випадок,
   * коли емодзі розрізняє пункти, а не прикрашає.
   */
...
(City or spot for filtering)`,
  ...
  // Тексти кандидаткам переїхали до constants/candidate-texts.ts (09.09.2026).
  // Вони жили тут серед адмінських рядків і тому не потрапили під вичитку
  // воронки: залишалися на «ти», з емодзі на поганих новинах. Нових
  // кандидатських рядків у цьому файлі бути не повинно — див. AGENTS.md.
...
Якщо якийсь день не підійде, зможеш попросити підміну прямо в графіку.`,
  ...
  /**
   * Сповіщення про нове завдання — спільне для всіх трьох майстрів
   * постановки (з дашборду задач, з профілю співробітниці, масове). Раніше
   * кожен майстер малював власний текст, і той самий факт звучав по-різному
   * залежно від того, яким адмінським екраном скористалися.
   *
   * Формулювання взяте з дашборду/масового майстра (було в task-creation.ts
   * і task-bulk.ts, ідентичне байт-у-байт) — воно тепліше й персональніше
   * («Бажаю успіхів! Ти впораєшся!»), ніж сухіше "📋 Нове завдання на
   * {date}!" з task-flow.ts. Дедлайн і completionHint — окремі опційні шматки,
   * щоб не плодити варіанти самого шаблону.
   *
   * Рівно один емоційний емодзі на весь текст (💖 в кінці) — правило
   * тестується в staff-texts-tone.test.ts. 📅 і ⏰ — маркери полів, а не
   * прикраса, тож заголовок і підказка про підтвердження обходяться без
   * власних емодзі.
   */
  "staff-task-notification": (p: { text: string; date: string; deadlineTime?: string | null; completionHint?: boolean }) => {
    const deadlineText = p.deadlineTime ? `\n⏰ Дедлайн: ${p.deadlineTime}` : "";
    const completionHint = p.completionHint
      ? `\n\n<b>Для завершення потрібно надіслати підтвердження в розділі «Мої завдання».</b>`
      : "";
    return `<b>Нове завдання!</b>\n\n${p.text}\n\n📅 Дата: ${p.date}${deadlineText}${completionHint}\n\nБажаю успіхів! Ти впораєшся! 💖`;
  },
  "staff-task-notification-btn-tasks": `📋 Мої завдання`,
