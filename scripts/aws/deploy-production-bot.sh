...
AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED="${AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED:-false}"
AWS_PARCELS_CANONICAL_READ_ENABLED="${AWS_PARCELS_CANONICAL_READ_ENABLED:-false}"
AWS_ACCESS_REVOCATIONS_ENABLED="${AWS_ACCESS_REVOCATIONS_ENABLED:-false}"
AWS_DDS_API_WRITE_ENABLED="${AWS_DDS_API_WRITE_ENABLED:-false}"
AWS_RECRUITING_MIRROR_ENABLED="${AWS_RECRUITING_MIRROR_ENABLED:-false}"
AWS_RECRUITING_SLOTS_ENABLED="${AWS_RECRUITING_SLOTS_ENABLED:-false}"
AWS_RECRUITING_COMMANDS_ENABLED="${AWS_RECRUITING_COMMANDS_ENABLED:-false}"
AWS_RECRUITING_TATTOO_WEB_ENABLED="${AWS_RECRUITING_TATTOO_WEB_ENABLED:-false}"
...
if [[ "$AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED" != "true" && "$AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED" != "false" ]]; then
  echo "AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_PARCELS_CANONICAL_READ_ENABLED" != "true" && "$AWS_PARCELS_CANONICAL_READ_ENABLED" != "false" ]]; then
  echo "AWS_PARCELS_CANONICAL_READ_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_ACCESS_REVOCATIONS_ENABLED" != "true" && "$AWS_ACCESS_REVOCATIONS_ENABLED" != "false" ]]; then
  echo "AWS_ACCESS_REVOCATIONS_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_DDS_API_WRITE_ENABLED" != "true" && "$AWS_DDS_API_WRITE_ENABLED" != "false" ]]; then
  echo "AWS_DDS_API_WRITE_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_RECRUITING_MIRROR_ENABLED" != "true" && "$AWS_RECRUITING_MIRROR_ENABLED" != "false" ]]; then
  echo "AWS_RECRUITING_MIRROR_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_RECRUITING_SLOTS_ENABLED" != "true" && "$AWS_RECRUITING_SLOTS_ENABLED" != "false" ]]; then
  echo "AWS_RECRUITING_SLOTS_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_RECRUITING_COMMANDS_ENABLED" != "true" && "$AWS_RECRUITING_COMMANDS_ENABLED" != "false" ]]; then
  echo "AWS_RECRUITING_COMMANDS_ENABLED must be true or false." >&2
  exit 1
fi
if [[ "$AWS_RECRUITING_TATTOO_WEB_ENABLED" != "true" && "$AWS_RECRUITING_TATTOO_WEB_ENABLED" != "false" ]]; then
  echo "AWS_RECRUITING_TATTOO_WEB_ENABLED must be true or false." >&2
  exit 1
fi
...
set_env AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED "$AWS_REPLACEMENT_AUTO_CONFIRM_ENABLED"
set_env AWS_PARCELS_CANONICAL_READ_ENABLED "$AWS_PARCELS_CANONICAL_READ_ENABLED"
set_env AWS_ACCESS_REVOCATIONS_ENABLED "$AWS_ACCESS_REVOCATIONS_ENABLED"
set_env AWS_DDS_API_WRITE_ENABLED "$AWS_DDS_API_WRITE_ENABLED"
set_env AWS_RECRUITING_MIRROR_ENABLED "$AWS_RECRUITING_MIRROR_ENABLED"
set_env AWS_RECRUITING_SLOTS_ENABLED "$AWS_RECRUITING_SLOTS_ENABLED"
set_env AWS_RECRUITING_COMMANDS_ENABLED "$AWS_RECRUITING_COMMANDS_ENABLED"
set_env AWS_RECRUITING_TATTOO_WEB_ENABLED "$AWS_RECRUITING_TATTOO_WEB_ENABLED"
