/**
 * Точечный бэкфилл зеркала: пушит только кандидаток на ревью внешности
 * (MANUAL_REVIEW). Копия backfill-recruiting-mirror.ts с одним добавленным
 * `where` — остальное не тронуто сознательно: импорты с фолбэком на dist,
 * идемпотентный пуш и пауза уже проверены боем.
 *
 * Зачем: разовый подъём анкет писал через updateMany, мимо методов
 * репозитория, к которым привязано зеркалирование. Данные в боте верные,
 * вебапп про них не знает. Полный бэкфилл (3598 строк) ради 19 — избыточен.
 *
 * Исходный бэкфилл пушит ВСЕХ кандидатов бота в вебапп
